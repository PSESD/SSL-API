...
var urlLive = 'https://auth.studentsuccesslink.org';
var url = 'http://localhost:3000';
var urlApiLive = 'https://api.studentsuccesslink.org';
...
describe('Migrate Production', function () {
    ...
    var orgData = {
        ...
        "description": null,
        "website": "studentsuccesslink.org",
        "url": "ssldemo.studentsuccesslink.org",
        "externalServiceId": 5,
        "personnelId": 1,
    ...
    describe('Oauth2', function () {
        ...
        it('user should add a new client', function (done) {
            request(url).post('/api/clients')
                ...
                .send({
                    ...
                    name: 'studentsuccesslink.org',
                    redirect_uri: '\\*\\.studentsuccesslink\\.org'
                })
                .expect('Content-Type', /json/)
                .expect(200)
                .expect(function (res) {
                    ...
                    assert.equal('studentsuccesslink.org', res.body.client_id);
                    assert.equal('studentsuccesslink.org', res.body.name);
                    assert.equal('\\*\\.studentsuccesslink\\.org', res.body.redirect_uri);
        ...
        it('user should be able to list clients', function (done) {
            request(url).get('/api/clients')
                ...
                .expect(function (res) {
                    var o = res.body[0];
                    assert.equal('studentsuccesslink.org', o.client_id);
                    assert.equal('studentsuccesslink.org', o.name);
                    assert.equal('\\*\\.studentsuccesslink\\.org', o.redirect_uri);
                })
                .end(done);
        ...
        it('get a token', function (done) {
            ...
            request(url).post('/api/oauth2/token')
                .auth('studentsuccesslink.org', secretCode)
                .expect('Content-Type', /json/)
                .type('form')
                .send({
                    username: newUser.email,
